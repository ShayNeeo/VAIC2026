"""Contract tests for Approval Agent (V3 signature)."""

import pytest
from servers.approval_agent.server import (
    issue_token,
    verify_token,
    health_check,
    IssueTokenRequest,
...
class TestApprovalAgentContract:
    ...
    async def test_app_01_issue_token_returns_hmac_signed(self):
        ...
        result = await issue_token(IssueTokenRequest(
            case_id="CASE-001", rm_id="RM-001",
            permissions=["create_crm_case"], payload={"action": "create_case", "data": {}},
        ))
    ...
    async def test_app_02_verify_token_valid(self):
        ...
        issue = await issue_token(IssueTokenRequest(
            case_id="CASE-002", rm_id="RM-001",
            permissions=["create_crm_case"], payload={"action": "create_case", "data": {}},
        ))
        verify = await verify_token(VerifyTokenRequest(
            token=issue["token"], case_id="CASE-002", rm_id="RM-001",
    ...
    async def test_app_03_verify_wrong_case_rejected(self):
        ...
        issue = await issue_token(IssueTokenRequest(
            case_id="CASE-A", rm_id="RM-001",
            permissions=["create_crm_case"], payload={"action": "create_case"},
        ))
        verify = await verify_token(VerifyTokenRequest(
            token=issue["token"], case_id="CASE-B", rm_id="RM-001", payload={"action": "create_case"},
        ))
    ...
    async def test_app_04_verify_wrong_rm_rejected(self):
        ...
        issue = await issue_token(IssueTokenRequest(
            case_id="CASE-003", rm_id="RM-001",
            permissions=["create_crm_case"], payload={"action": "create_case"},
        ))
        verify = await verify_token(VerifyTokenRequest(
            token=issue["token"], case_id="CASE-003", rm_id="RM-002", payload={"action": "create_case"},
        ))
        assert verify["valid"] is False
    ...
    async def test_app_05_verify_tampered_payload_rejected(self):
        ...
        issue = await issue_token(IssueTokenRequest(
            case_id="CASE-004", rm_id="RM-001",
            permissions=["create_crm_case"], payload={"action": "create_case", "amount": 100},
        ))
        verify = await verify_token(VerifyTokenRequest(
            token=issue["token"], case_id="CASE-004", rm_id="RM-001",
    ...
    async def test_app_06_verify_reused_token_rejected(self):
        ...
        issue = await issue_token(IssueTokenRequest(
            case_id="CASE-005", rm_id="RM-001",
            permissions=["create_crm_case"], payload={"action": "create_case"},
        ))
        await verify_token(VerifyTokenRequest(
            token=issue["token"], case_id="CASE-005", rm_id="RM-001", payload={"action": "create_case"},
        ))
        verify = await verify_token(VerifyTokenRequest(
            token=issue["token"], case_id="CASE-005", rm_id="RM-001", payload={"action": "create_case"},
        ))
